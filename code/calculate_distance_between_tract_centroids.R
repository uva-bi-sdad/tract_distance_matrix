...
setDT(va_tracts_centroids_2020)

va_tracts_2019 <- tigris::tracts(state = "VA", year = 2019, cb = T)
va_tracts_centroids_2019 <- sf::st_centroid(va_tracts_2019)
va_tracts_centroids_2019$latlons <- paste0(st_coordinates(va_tracts_centroids_2019$geometry)[, "Y"], "+", st_coordinates(va_tracts_centroids_2019$geometry)[, "X"])
setDT(va_tracts_centroids_2019)

all_tracts_2019 <- va_tracts_centroids_2019[, .(geoid = GEOID, latlons)]
old_tracts_2019 <- merge(va_tracts_centroids_2019, va_tracts_centroids_2020, by = "GEOID", all.x = T)[is.na(TRACTCE.y), .(geoid = GEOID, latlons = latlons.x)]
...
od <- read.csv(xzfile("data/va_unique_lonlat_combos.csv.xz"))
...
# For 2019
#for (i in 2:nrow(old_tracts_2019)) {
for (i in 211:274) {
  o1 <- as.double(gsub("(.*)\\+(.*)", "\\2", old_tracts_2019[i,2]))
  o2 <- as.double(gsub("(.*)\\+(.*)", "\\1", old_tracts_2019[i,2]))
  o <- sf::st_as_sf(data.frame(lon=o1, lat=o2), coords = c("lon", "lat"), crs = st_crs(4326))
  
  print(old_tracts_2019[i,1])
  
  chunckNum <- ceiling(nrow(all_tracts_2019)/100)
  chunkSize <- 100
  # xo <- (0*chunkSize+1)
  if (exists("timedist_fnl")) rm(timedist_fnl)
  for (j in 1:chunckNum) {
    x <- ((j-1)*chunkSize+1)
    y <- min(nrow(all_tracts_2019),(j*chunkSize))
    
    d1 <- as.double(gsub("(.*)\\+(.*)", "\\2", all_tracts_2019[x:y][[2]]))
    d2 <- as.double(gsub("(.*)\\+(.*)", "\\1", all_tracts_2019[x:y][[2]]))
    d <- sf::st_as_sf(data.frame(lon=d1, lat=d2), coords = c("lon", "lat"), crs = st_crs(4326))
    
    timedist <- osrm::osrmTable(o, d, measure = c("distance", "duration"))
    
    ret <- data.table(origin = old_tracts_2019[i,2], dest = all_tracts_2019[x:y,2])
    ret$dist <- round(timedist$distances,0)*1000
    ret$time <- round(timedist$durations,0)*60
    ret <- ret[dist != 0]
...
# Combine osrmtable outputs for 2019
if (exists("osrmtablecombined")) rm(osrmtablecombined)
tract_files <- list.files("data/osrm_table/", pattern = "*.csv$", full.names = T)
for (f in tract_files) {
  dt <- data.table::fread(f)
  if (!exists("osrmtablecombined")) osrmtablecombined <- dt else osrmtablecombined <- data.table::rbindlist(list(osrmtablecombined, dt))
}

# Combine with 2020 tract distances
td2020 <- read.csv(xzfile("data/osrm_table/tract_distances_combined.csv.xz"))
colnames(td2020) <- colnames(osrmtablecombined)
combine1920 <- rbindlist(list(td2020, osrmtablecombined), use.names = T)

# Combine 19 and 20 centroid files
va_tracts_centroids_2019_2020 <- rbindlist(list(va_tracts_centroids_2020[, .(GEOID, latlons)], va_tracts_centroids_2019[, .(GEOID, latlons)]))

# Merge to add tract ids
tract_ids_lat_lons <- va_tracts_centroids_2019_2020
m1 <- merge(tract_ids_lat_lons, combine1920, by.x = "latlons", by.y = "origin.latlons", all.x = T)
m2 <- merge(m1, tract_ids_lat_lons, by.x = "dest.latlons", by.y = "latlons", all.x = T)
m2 <- m2[!is.na(dest.latlons)]
setorder(m2, latlons)

m_fnl <- m2[, .(tract_orig = GEOID.x, tract_dest = GEOID.y, tract_orig_coords = latlons, tract_dest_coords = dest.latlons, dist_meters = dist/1000, time_mins = time/60)]
write.csv(m_fnl, xzfile("data/osrm_table/tract2tract_dist_time.csv.xz"))
...
m_fnl_5 <- m_fnl[4000001:5000000,]
m_fnl_6 <- m_fnl[5000001:nrow(m_fnl)]
